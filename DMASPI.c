...
void DMA_Init(uint16_t period) {
    ...
    UDMA_CHMAP1_R = (UDMA_CHMAP1_R & 0xFFFFFFF0) | 0x00000003; // timer5A - Channel 8 enc 3
...
void static setRegular(void) {
    ...
    /* DMACHCTL          Bits    Value Description
       ...
       XFERMODE          2:0     011   Use auto transfer mode
      */
}
...
// The source address is incremented by 4 each 32-bit transfer, destination address is fixed
...
void DMA_Start(uint16_t *source, volatile uint32_t *destination, uint32_t count) {
    ...
    Count = count;  // number of bytes to transmit
    setRegular();
    setAlternate();
...
void Timer5A_Handler(void) { // interrupts after each block is transferred
    ...
    if ((ucControlTable[CH8 + 2] & 0x0007) == 0) { setRegular(); }  // rebuild channel control structure
    // Alternate buffer complete
    if ((ucControlTable[CH8ALT + 2] & 0x0007) == 0) { setAlternate(); }                        // rebuild channel control structure
    //PF2 ^= 0x04;
}
